...
struct
  ...
  fun match {usage = {short, long, desc}, arg} =
    fn [] => NONE
     ...
     | Flag other :: rest =>
      if "--" ^ long <> other andalso Option.getOpt (Option.map (fn s => shortToString s <> other) short, true) then NONE
      else
        SOME (case (arg, rest) of
